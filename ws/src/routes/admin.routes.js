   ...
   router.post("/materias/novo", async (req, res) => {
    ...
    if (materiaExistente) {
      if (materiaExistente.cod_materia === novaMateria.cod_materia) {
        req.flash("error_msg", "Já existe uma matéria cadastrado com este código");
      } else {
  ...
  router.post("/professores/novo", async (req, res) => {
    ...
    const schema = Joi.object({
      ra: Joi.string().required().min(10).max(10).messages({
        ...
        'string.min': 'O RA deve possuir 10 caracteres, incluindo o ponto e traço',
        'string.max': 'O RA deve possuir 10 caracteres, incluindo o ponto e traço'
      }),
